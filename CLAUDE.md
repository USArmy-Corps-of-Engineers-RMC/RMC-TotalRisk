...
| `RMC.TotalRisk.Core.Interfaces` | Every interface: `IRiskFunction`, `IHazardFunction`/`IUnivariateHazardFunction`/`IBivariateHazardFunction`, `ITransformFunction`/`IBivariateTransformFunction`, `IResponseFunction`, `IConsequenceFunction`/`IBivariateConsequenceFunction`, `IRiskFunctionResolver`, `IRiskElement`, `IRiskElementNameAuthority`; `IAnalysis` (Phase 4+) |
...
| Later | RFAHazard, `CompositeHazard` parameter-set import, remaining bivariate (S4–S7: response, graph, engine, verification), BestFit imports, LifeSim | — | Phases 9 follow-on, 11 |
