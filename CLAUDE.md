...
| `RMC.TotalRisk.RiskFunctions.Responses.Trees` | Common branching-response contracts: `TreeLinkMode`, `TreeDeletePolicy`, `TreeNodeReference`, `ResponseBranchDescriptor`, `ResponseBranchSample`, `IBranchingResponseFunction` (Phase 10A) |
...
| Trees | TreeNodeReference / response-branch contracts / TreeLinkMode / TreeDeletePolicy | P/T | immutable reference and exhaustive branch-sample contracts + enum pins (Phase 10A first slice, landed 2026-07-28) |
| Response | EventTreeResponse + EventTree + EventTreeLinkNode + EventNodeBase / InitiatingNode / ChanceNode / RemainderNode / ProbabilitySource | P/T/V | two vertical slices: legacy path-product/remainder/normalization algebra; scalar/aligned-table/ordinary-response sources; controlled core authoring/traversal; internal/external `IndependentClone` occurrence expansion; resolver-backed two-mode XML; cross-function cycle diagnostics; occurrence-aware sampling; projected hash/seed identity; analytic/indexed/link verification (`EventTreeVerification`, 5 tests; `docs/verification/event-tree.md`). Phase 10A remains open for the remaining authoring/topology surface, probability-source recursion, legacy conversion/templates, expanded graph ports, cached compiled performance, and the remaining verification gates. |
