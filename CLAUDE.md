...
| Systems | SystemComponent (graph-owned; projection + identity hash + occurrence indices + MVN) | P/T/V | engine scenarios + seed-bug regressions (Phases 4–5) + the multi-component system matrix incl. identical-content occurrence-index scenarios and the `HazardThreshold` API surface (Phase 6 — landed 2026-07-23) |
...
- `EmpiricalDistribution` does not round-trip its X/P tables through the base `ToXElement()` (fix planned upstream in Phase 8, item N5 — until then serialize the underlying paired data).
...
- **The Vegas power-transform Jacobian IS folded into the weight handed to the integrand** (`Vegas.cs:466-472`) — the recorded `wgt` mass is Jacobian-corrected; the 4b γ-audit (`SystemRiskVerification.Test_JointSystem_TailFocusAudit`) is green, so γ > 1 is safe to enable (N9 stays the upstream unit-test item).
