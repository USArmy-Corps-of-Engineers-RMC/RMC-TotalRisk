...
| `RMC.TotalRisk.RiskFunctions.Responses.Trees` | Common branching-response contracts: `TreeLinkMode`, `TreeDeletePolicy`, `TreeNodeReference`, `ResponseBranchDescriptor`, `ResponseBranchSample`, `IBranchingResponseFunction` (Phase 10A first slice) |
| `RMC.TotalRisk.RiskFunctions.Responses.EventTrees` | `EventTreeResponse`, `EventTree`, `EventNodeBase`, `InitiatingNode`, `ChanceNode`, `RemainderNode`, `ProbabilitySource` (Phase 10A first slice; links and remaining foundation still open) |
...
| Response | EventTreeResponse + EventTree + EventNodeBase / InitiatingNode / ChanceNode / RemainderNode / ProbabilitySource | P/T/V | first vertical slice: legacy path-product/remainder/normalization algebra, scalar/aligned-table/ordinary-response sources, controlled core authoring/traversal, branch outputs, dual-mode XML, projected hashes, and analytic/indexed verification (`EventTreeVerification`, 3 tests; `docs/verification/event-tree.md`). Phase 10A remains open for links/clones, remaining authoring/topology, recursive cycles, legacy conversion, graph ports, compiled performance, and remaining verification gates. |
