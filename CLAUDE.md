...
| `RMC.TotalRisk.Core.Interfaces` | Every interface: `IRiskFunction`, `IHazardFunction`/`IUnivariateHazardFunction`/`IBivariateHazardFunction`, `ITransformFunction`, `IResponseFunction`, `IConsequenceFunction`, `IRiskFunctionResolver`, `IRiskElement`, `IRiskElementNameAuthority`; `IAnalysis` (Phase 4+) |
...
| Later | RFAHazard, `CompositeHazard` parameter-set import, remaining bivariate (S3–S7: tables, response, graph, engine, verification), BestFit imports, LifeSim | — | Phases 9 follow-on, 11 |
