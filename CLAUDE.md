...
| Response | EventTreeResponse + EventTree + EventTreeLinkNode + EventNodeBase / InitiatingNode / ChanceNode / RemainderNode / ProbabilitySource | P/T/V | five vertical slices: legacy path-product/remainder/normalization algebra; scalar/aligned-table/ordinary-response sources; controlled transactional authoring including fresh-ID copy/paste, replace, materialize, delete-materialize, and prune; deterministic expanded topological/unreachable/internal/external reference queries; internal/external `IndependentClone` occurrence expansion; direct and multi-level `EventTreeResponse` probability sources evaluated at caller hazards; exact recursive sampler discovery through nested responses/tables/links; independent repeated-occurrence streams; indexed/percentile/LHS propagation; resolver-backed two-mode XML and nested reference repair; complete structural/source/mixed cycle paths; transactional sampler rollback; projected recursive hash/seed identity; and import-only conversion of the recursive v1.0 `Node` shape (direct or wrapped), both hazard-attribute/GUID spellings, scalar/table/name-only-response sources, deterministic diagnostics, and current-only writes. The legacy `TestIO` shape plus exact `Basic` and `Concrete Dam Gate Failure` templates are verified terminal-for-terminal (`EventTreeVerification`, 10 tests; `docs/verification/event-tree.md`). Phase 10A remains open for expanded per-leaf graph output ports and stale-connection policy; immutable compiled-plan caching/invalidation; large-tree performance; property-based testing; branch-routing Monte Carlo; graph-connected per-leaf consequence verification; aggregate LHS variance reduction; and the remaining thread-count, coverage, and performance exit gates. |
