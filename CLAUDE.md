...
| `RMC.TotalRisk.RiskFunctions.Responses.Trees` | Common branching-response contracts: `TreeFragment`, `TreeLinkMode`, `TreeDeletePolicy`, `TreeNodeReference`, `ResponseBranchDescriptor`, `ResponseBranchSample`, `IBranchingResponseFunction`, `ProbabilitySource` + `ProbabilitySourceKind` (the shared tree probability-source pair, promoted at Phase 10B slice 0a) (Phase 10A) |
...
| Trees | TreeFragment / TreeNodeReference / response-branch contracts / TreeLinkMode / TreeDeletePolicy | P/T | immutable fragment/reference/branch-sample contracts + enum pins; fragment source IDs are authoring-only remap metadata, never serialized or hashed (Phase 10A first and third slices, landed 2026-07-28) |
