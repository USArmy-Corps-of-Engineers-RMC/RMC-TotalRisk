...
| Hazard | TabularHazard | P/T/V | NFIP assurance deterministic dense-tabulation variant vs exact quadrature (`NfipAssuranceVerification`; docs/verification/nfip-assurance.md) |
...
| Transform | TabularTransform | P/T/V | NFIP rating-curve chain (log-flow interpolation) in every active assurance scenario (`NfipAssuranceVerification`) |
...
| Verification | Legacy Dev suite + 2024 report scenario matrix | V | 142 source test methods classified (including event-tree `TestIO`); 113 applicable methods mapped to current tests; all 49 report system/joint-failure scenarios covered; FDA/NFIP explicitly obsolete (`legacy-traceability.csv`, repository validator) |
...
- **`CostBenefitAnalysis`** — composite analysis owning a `List<RiskAnalysis>` of alternatives; the Phase 4 self-contained-analysis foundation is its design driver. Scope includes the only cost-benefit math v1.0 shipped: `PlanRow.EquivalentAnnual` (equivalent annual damages — year-interpolated EAD between base and future condition, discounted at the plan rate, annualized over the period of analysis; lived in the v1.0 UI Risk Comparison surface, algorithmically headless).
