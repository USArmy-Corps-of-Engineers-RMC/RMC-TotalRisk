...
| Response | EventTreeResponse + EventTree + EventTreeLinkNode + EventNodeBase / InitiatingNode / ChanceNode / RemainderNode / ProbabilitySource | P/T/V | three vertical slices: legacy path-product/remainder/normalization algebra; scalar/aligned-table/ordinary-response sources; controlled transactional authoring including fresh-ID copy/paste, replace, materialize, delete-materialize, and prune; deterministic expanded topological/unreachable/internal/external reference queries; internal/external `IndependentClone` occurrence expansion; resolver-backed two-mode XML; cross-function cycle diagnostics; occurrence-aware sampling; projected hash/seed identity; analytic/indexed/link verification (`EventTreeVerification`, 5 tests; `docs/verification/event-tree.md`). Phase 10A remains open for probability-source recursion, legacy conversion/templates, expanded graph ports and stale-connection policy, cached compiled performance, and the remaining verification gates. |
