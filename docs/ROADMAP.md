...
| 11 | Bivariate risk analysis | Design ratified (2026-08-03) — [requirements/BIVARIATE_RISK_DESIGN.md](requirements/BIVARIATE_RISK_DESIGN.md); implementation sessions S1–S7 pending |
...
## Phase 11 — Bivariate risk analysis
...
**Exit criteria:** the four bivariate types P/T/V; the five reserved gates open with the full validation matrices; F1 byte-gate hash unchanged; F8 baseline committed; both new families green run isolated; traceability rows for `Test_BivariateRisk.vb`/`Test_DAMRAE.vb` re-classified Covered; the S7 documentation amendments landed (arch doc §6.1.2/§6.5/§7.4 rewrites + §5.8.4 rows + Q-Q resolution, `docs/technical-reference/bivariate-hazards.md`, verification pages, CLAUDE.md matrix/gotchas).
