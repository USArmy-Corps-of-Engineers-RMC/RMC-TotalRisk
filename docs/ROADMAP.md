...
## Phase 3 — Risk components + results containers

**Scope:** `Models/RiskAnalysis/Components`: `SystemComponent`, `FailureMode` (concrete classes — no interfaces; v1.0 surfaces preserved incl. ctor defaults, `FailureModes` collection semantics, `FailureModeDependency` property of type `DependencyType`, `CorrelationMatrix double[,]` with Cholesky validation, MVN builds with `1−√εmach` / `−1/(D−1)+√εmach` off-diagonals), `SampledComponent`, `SampledFailureMode` (preserving the v1.0 coupling: failure and non-failure consequences share one draw per realization), `ComponentRiskOutput`, and the legacy-named enums `DependencyType`, `FailureModeMethod`, `JointConsequenceType`, `SystemRiskType`, `RiskType` (standalone files). Occurrence-index assignment (`AssignOccurrenceIndices`, arch doc §5.5.4). Name-based `FromXElement(XElement, IProject)` resolution is replaced by self-contained inline child serialization.

`Models/RiskAnalysis/Results`: **JSON-first redesign** (the sanctioned v1.0 relaxation — v1.0 stored `SystemRealization` + `EnsembleResults` as compressed BinaryFormatter BLOBs in five columns; BinaryFormatter is removed from .NET 10). v1.1 containers (`Curve`, `Curves`, `RiskPoint`, `Ensemble`, `ComponentRealization`/`FailureModeRealization`/`SystemRealization`, `ComponentResults`/`FailureModeResults`/`SummaryRiskResults`/`SystemRiskResults`/`EnsembleResults`) get explicit public serializable state (v1.0's `Curve` hid moments/bin parameters in private fields), System.Text.Json contracts with `ToJson()`/`FromJson()` (+ compressed-bytes overloads), and the same computed outputs (total probability, conditional mean, moments, VaR/CVaR, LEC/hazard-frequency/hazard-vs-CEN profiles). Anticipate `ReliabilityAnalysis` summary shapes (failure probability / AFP without consequences). v1.0 result BLOBs are not readable — old projects re-run (v1.0 already silently degraded unreadable results to not-estimated).

**Unit tests:** occurrence-index scheme (identical-content components get 0..n−1 in canonical order; cross-analysis stability tuples), component/FM validation matrices, correlation-matrix serialization (Q-G resolution), results container JSON round-trips, moment/risk-measure computations at known values.
