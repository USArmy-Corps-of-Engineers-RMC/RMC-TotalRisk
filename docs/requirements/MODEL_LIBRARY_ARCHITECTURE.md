...
**Status**: 2026-07-23 — **v0.16** (Phase 6.5 landing; supersedes conflicting text below wherever it appears). The gap-audit closures from the multi-consequence session:
...
- **Q-W** *(added 2026-07-21, v0.13; **narrowed 2026-07-23, v0.16**)*: *Shared exposure state* across consequence types. In reality a single day/night draw should drive **all** consequence types on that mode at once (economic and life loss share the same exposure state); v1.0 has no concept of this. **v0.16 erratum**: the engine computes per-type marginal results and never crosses branches across types — the §6.4.1 cross-type product never existed in the compute, so the mode guardrail is the branch **sum** across positions (64/1024 thresholds) and the question is moot for every output produced today. Design the per-mode shared-exposure declaration when (and only when) cross-type joint statistics are wanted — scoped to Phase 6.6.
- **Q-X** *(added 2026-07-22, v0.14; **successor design RATIFIED 2026-07-23, v0.16**)*: Multi-stage response composition semantics. The Phase-3 grammar `T* (R T*)* C` authors chains with two or more response stages, but v1.0 had exactly one response and no oracle covers composition. **Ratified deferral (v0.14)**: end users want event-tree branch semantics, not a stage-probability product, so the engine gate (`RiskAnalysis.Validate()`) rejects multi-stage modes; `SampledFailureMode`'s constructor throw is the placeholder seam. **Resolution path (v0.16)**: the cascading-response-end-states design (status block item 4 — typed Fail/Non-Fail output ports, per-stage branch polarity, end states as projected modes in mutually-exclusive state groups with auto-remainder to background) IS the ratified composition semantics; the two rejection seams are replaced at Phase 6.7, where Q-X closes.
