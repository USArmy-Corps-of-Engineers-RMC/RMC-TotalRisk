...
| **N11** | Pooled `IndependentExclusive` outputs + truncation diagnostic | Risk-engine follow-up (TotalRisk Phases 6/6.5, ratified into 2.2.0 on 2026-07-25). (a) **Allocation pooling:** additive caller-supplied-buffer overloads of the pooled `Probability.IndependentExclusive` (TotalRisk's joint path calls it per VEGAS evaluation and the out-lists allocate every call), mirrored on `PositivelyDependentExclusive`/`ExclusivePCM`/`ExclusiveMVN` where trivial. (b) **Truncation honesty:** the 1e-4 abs+rel inclusion–exclusion early-return appends a closing pseudo-row and silently truncates the deepest combinations (TotalRisk's mass-balance witnesses observe it at D > 2); tolerances are already parameters — add an observable diagnostic (truncated-mass/flag output) with **no default behavior change** (Hydrologics consumes these kernels too). |
