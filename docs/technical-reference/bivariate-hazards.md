...
The third regime is the practitioner warning: when the response probability spans orders of magnitude across the secondary axis AND the secondary marginal carries a normal-Z (or similarly heavy) probability transform, the conditional integrand's mass hides in the last few bins of t, and the uniform-t trapezoid needs hundreds of bins for percent-level accuracy. Raise `SecondaryIntegrationBins` toward its 1000 cap for such fixtures — and measure rather than guess, with the built-in diagnostic below.

### Estimating the discretization error a posteriori

`RiskAnalysis.EstimateSecondaryDiscretizationError(componentIndex)` turns the bin-count question from a modelling judgment into a reported number: the mean pass is integrated at the configured, halved, and quartered bin counts (on diagnostic snapshots — the stored count, the canonical hash, and every seed are untouched, and nothing is serialized), and the annual failure probability plus the mean incremental risk per consequence type are Richardson-extrapolated at the trapezoid's second-order rate. Each `DiscretizationEstimate` reports the three values, the extrapolated limit, the estimated relative error of the configured count, and the **observed convergence ratio** — the regime check. Near four, the second-order rate is in force and the estimate is trustworthy (the smooth regime above); well below four, the estimate is indicative only and the fixture is in the tail-concentrated regime, where more bins or the collapse arrangement deserve consideration. The verification family pins the diagnostic against the convergence study's measured true errors: in the smooth regime the estimate lands within a factor of two of the truth with the ratio at four, and on the legacy tail-concentrated fixture the ratio falls visibly below four — flagging exactly the fixture the study flagged. The configured count needs at least twelve bins for the three levels; the diagnostic re-runs the component's sampler setup like every stored-results diagnostic.
