...
| [Engine reproducibility](engine-reproducibility.md) | `EngineReproducibilityVerification` | The v1 seed-dependency bug regression: repeated-run, metadata-edit, and round-trip bit-identity of full results JSON; extended 2026-08-07 with the bivariate pins (thread counts with conditional bins, marginal-link rename + both serialization modes, unrelated-growth seed stability, and the pinned-seed secondary perturbation both ways) | ✅ Verified (2026-08-07) |
...
| [Bivariate risk](bivariate-risk.md) | `BivariateRiskVerification` | Legacy `Test_Bivariate_Risk` ported (seed 45678, N = 10⁶, the pinned pga/stage/rnd draw order) vs the conditional-bin engine at 1000 and 20 bins; the exact union-grid SRP closed form with the re-captured legacy 100M cross-check; the re-anchored PFM-08 chained-bilinear oracle; the collapse-versus-joint consistency cross-anchor with interpolant-preserving refinement on both paths; and the axis-choice measurement (stage-primary with PGA collapsed lands 0.241% from exact against 35.4% for the default-bin bivariate arrangement) | ✅ Verified (2026-08-07) |
| [Copula dependence](copula-dependence.md) | `CopulaDependenceVerification` | Greenfield family: independence trapezoid exactness, Normal/Clayton analytic anchors (h-functions, inverse conditionals, and the E[UV] = ∫∫C dense re-derivation), the three-fixture bin-convergence study that pins the family's discretization allowances, the Gumbel tail-orientation pin, and posterior-injected marginal realization parity | ✅ Verified (2026-08-07) |
