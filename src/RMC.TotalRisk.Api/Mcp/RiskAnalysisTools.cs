...
namespace RMC.TotalRisk.Api.Mcp
{
    ...
    public class RiskAnalysisTools
    {
        ...
        [Description("Compute a complete dam/levee risk analysis synchronously and return the mean risk results: loss exceedance curves, annualized failure probability, expected annual consequences, per-failure-mode UNADJUSTED marginal and combination-ADJUSTED curves, and risk contributions. The request is a self-contained JSON definition (hazard stage-frequency table, failure modes with optional hazard-to-response transform chains — e.g. stage-to-depth for overtopping or stage-to-discharge for spillway erosion, with consequenceHazardPosition selecting the axis the consequences read — fragility tables, day/night consequence mixtures, engine settings) — the same contract as POST /api/risk-analyses/compute. Nothing is stored between calls. Mean-only deterministic runs complete in well under a second. Call get_metadata for enums/defaults/conventions and get_example_request for a valid filled template; validate first with validate_risk_analysis when constructing a payload incrementally.")]
        public async Task<string> RunRiskAnalysis(
            [Description("The complete risk-analysis definition: { name?, specifiedConsequence, consequenceUnit, components: [ { name, hazard: { exceedanceProbabilities (descending AEP), hazardValues (ascending) }, failureModes: [ { name, transforms?: [ { type: tabularTransform|linearTransform|powerTransform, transformedHazard, transformedHazardUnit, ... } ], response: { hazardValues, responseProbabilities } (keyed to the LAST transform's output axis when transforms are present), consequences: [ tabular or compositeMixture with branches [{ weight, function }] ], consequenceHazardPosition? (0 = the raw component hazard, the default; k = after the k-th transform) } ], nonFailConsequences: [...] } ], options?: { prngSeed?, riskMeasures?, outputAdjustedFailureModeCurves?, ... } }. See get_example_request for a complete valid instance.")]
        ...
        [Description("Get a filled example request: a single dam with a deterministic reservoir stage-frequency hazard, three failure modes (overtopping erosion, backward erosion piping, concentrated leak erosion), day/night exposure-weighted life-loss mixtures on every failure mode, and a day/night non-fail mixture. The overtopping mode demonstrates a hazard-to-response transform: a linear stage-to-overtopping-depth crest offset feeds a depth-keyed fragility while its consequences stay on the stage axis (consequenceHazardPosition 0). The payload is valid and computable as returned — edit it rather than building from scratch.")]
